{ pkgs, osConfig, ... }:
let
  proj = "infra/tf/omada-controller-vm";

  defaults = {
    name        = "omada-controller";
    adguard_url = "http://10.0.0.1:3000";
    ip_cidr     = "10.0.0.71/24";
...
in {
  ...
  home.file."${proj}/variables.tf".text = ''
    ...
    variable "ip_cidr"      { default = "10.0.0.71/24" }
  ...
  home.file."${proj}/outputs.tf".text = ''
    output "name" {
      value = proxmox_virtual_environment_vm.omada-controller.name
    }

    output "ipv4" {
      # first address of first NIC reported by the guest agent
      value = try(proxmox_virtual_environment_vm.omada-controller.ipv4_addresses[0][0], null)
    }

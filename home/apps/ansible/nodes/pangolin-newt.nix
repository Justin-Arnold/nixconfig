{ pkgs, ... }:
let
  proj = "infra/ansible/pangolin-newt";
in {
  ...
  home.file."${proj}/inventory.ini".text = ''
    [pangolin]
    pangolin-newt ansible_host=pangolin-newt.host.internal ansible_user=justin ansible_ssh_private_key_file=/home/justin/.ssh/pangolin-newt_host_key
  '';
  ...
  home.file."${proj}/setup-ssh.yml".text = ''
    ...
    - name: Setup SSH keys from 1Password Connect
      ...
      vars:
        ...
        op_ssh_key_item_name: "ssh-host-pangolin-newt"
        op_connect_token: "{{ lookup('env', 'OP_API_TOKEN') }}"
        ssh_key_path: "/home/justin/.ssh/pangolin-newt_host_key"
        ...
        - name: Test SSH connection to target host
          shell: |
            ...
            ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                ...
                justin@pangolin-newt.host.internal echo "SSH connection successful"
          register: ssh_test
          failed_when: false
  ...
  home.file."${proj}/site.yml".text = ''
    ...
    - name: Bootstrap NixOS host (Pangolin Newt)
      hosts: pangolin-newt
      gather_facts: false
      vars:
        flake_attr: "pangolin-newt"
  ...
  programs.zsh.shellAliases = {
    ans-pangolin-newt = "cd ~/${proj} && ./run-ansible.sh";
    ans-pangolin-newt-check = "cd ~/${proj} && ./run-ansible.sh --check";

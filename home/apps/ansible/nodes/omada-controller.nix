{ pkgs, ... }:
let
  proj = "infra/ansible/omada-controller";
in {
  ...
  home.file."${proj}/inventory.ini".text = ''
    [omada-controller]
    omada-controller.host.internal ansible_user=justin ansible_ssh_private_key_file=/home/justin/.ssh/omada-controller_host_key
  '';
  ...
  home.file."${proj}/setup-ssh.yml".text = ''
    ...
    - name: Setup SSH keys from 1Password Connect
      ...
      vars:
        ...
        op_ssh_key_item_name: "ssh-host-omada-controller"
        op_connect_token: "{{ lookup('env', 'OP_API_TOKEN') }}"
        ssh_key_path: "/home/justin/.ssh/omada-controller_host_key"
        ...
        - name: Get SSH key from 1Password if not present locally
          block:
            ...
            - name: Find vault ID for Lab 0118
              set_fact:
                vault   _id: "{{ vaults_response.json | selectattr('name', 'equalto', op_vault_name) | map(attribute='id') | first }}"
        ...
        - name: Test SSH connection to target host
          shell: |
            ...
            ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                ...
                justin@omada-controller.host.internal echo "SSH connection successful"
          register: ssh_test
          failed_when: false
  ...
  home.file."${proj}/site.yml".text = ''
    ...
    - name: Bootstrap NixOS host (omada-controller)
      hosts: omada-controller
      gather_facts: false
      vars:
        flake_attr: "omada-controller"
      ...
      tasks:
        ...
        - name: Verify omada-controller service status
          raw: systemctl status omada-controller || true
          register: omada_controller_status

        - name: Display omada-controller status
          debug:
            var: omada_controller_status.stdout_lines
  '';
  ...
  programs.zsh.shellAliases = {
    ans-omada-controller = "cd ~/${proj} && ./run-ansible.sh";
    ans-omada-controller-check = "cd ~/${proj} && ./run-ansible.sh --check";
    ans-omada-controller-setup = "cd ~/${proj} && ansible-playbook setup-ssh.yml";
  };

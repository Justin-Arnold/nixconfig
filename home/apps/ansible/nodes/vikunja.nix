...
in {
  ...
  home.file."${proj}/inventory.ini".text = ''
    [pangolin]
    vikunja ansible_host=vikunja.host.internal ansible_user=justin ansible_ssh_private_key_file=/home/justin/.ssh/vikunja_host_key
  '';
  ...
  home.file."${proj}/setup-ssh.yml".text = ''
    ...
    - name: Setup SSH keys from 1Password Connect
      ...
      vars:
        ...
        op_ssh_key_item_name: "ssh-host-vikunja"
        op_connect_token: "{{ lookup('env', 'OP_API_TOKEN') }}"
        ssh_key_path: "/home/justin/.ssh/vikunja_host_key"
        ...
        - name: Test SSH connection to target host
          shell: |
            ...
            ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                ...
                justin@vikunja.host.internal echo "SSH connection successful"
          register: ssh_test
          failed_when: false
  ...
  programs.zsh.shellAliases = {
    ans-vikunja = "cd ~/${proj} && ./run-ansible.sh";
    ans-vikunja-check = "cd ~/${proj} && ./run-ansible.sh --check";
    ans-vikunja-setup = "cd ~/${proj} && ansible-playbook setup-ssh.yml";
  };

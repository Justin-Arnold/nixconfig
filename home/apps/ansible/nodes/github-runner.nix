{ pkgs, ... }:
let
  proj = "infra/ansible/github-runner";
in {
  ...
  home.file."${proj}/inventory.ini".text = ''
    [pangolin]
    github-runner ansible_host=github-runner.host.internal ansible_user=justin ansible_ssh_private_key_file=/home/justin/.ssh/github-runner_host_key
  '';
  ...
  home.file."${proj}/setup-ssh.yml".text = ''
    ...
    - name: Setup SSH keys from 1Password Connect
      ...
      vars:
        ...
        op_ssh_key_item_name: "ssh-host-github-runner"
        op_connect_token: "{{ lookup('env', 'OP_API_TOKEN') }}"
        ssh_key_path: "/home/justin/.ssh/github-runner_host_key"
        ...
        - name: Test SSH connection to target host
          shell: |
            ...
            ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                ...
                justin@github-runner.host.internal echo "SSH connection successful"
          register: ssh_test
          failed_when: false
  ...
  home.file."${proj}/site.yml".text = ''
    ...
    - name: Bootstrap NixOS host (PR Previews)
      hosts: github-runner
      gather_facts: false
      vars:
        flake_attr: "github-runner"
  ...
  programs.zsh.shellAliases = {
    ans-github-runner = "cd ~/${proj} && ./run-ansible.sh";
    ans-github-runner-check = "cd ~/${proj} && ./run-ansible.sh --check";

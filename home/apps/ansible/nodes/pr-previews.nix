{ pkgs, ... }:
let
  proj = "infra/ansible/pr-previews";
in {
  ...
  home.file."${proj}/inventory.ini".text = ''
    [pangolin]
    pr-previews ansible_host=pr-previews.host.internal ansible_user=justin ansible_ssh_private_key_file=/home/justin/.ssh/pr-previews_host_key
  '';
  ...
  home.file."${proj}/setup-ssh.yml".text = ''
    ...
    - name: Setup SSH keys from 1Password Connect
      ...
      vars:
        ...
        op_ssh_key_item_name: "ssh-host-pr-previews"
        op_connect_token: "{{ lookup('env', 'OP_API_TOKEN') }}"
        ssh_key_path: "/home/justin/.ssh/pr-previews_host_key"
        ...
        - name: Test SSH connection to target host
          shell: |
            ...
            ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                ...
                justin@pr-previews.host.internal echo "SSH connection successful"
          register: ssh_test
          failed_when: false

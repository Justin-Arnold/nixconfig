{ pkgs, osConfig, ... }:
let
  proj = "infra/tf/vikunja-vm";

  defaults = {
    name    = "vikunja";
    ip_cidr = "10.0.0.88/24";
...
in {
  ...
  home.file."${proj}/variables.tf".text = ''
    ...
    variable "disk_size_gb" { default = 32 }
    variable "ip_cidr"      { default = "10.0.0.88/24" }
  ...
  home.file."${proj}/main.tf".text = ''
    ...
    resource "proxmox_virtual_environment_vm" "vikunja" {
    name      = var.name
    node_name = var.node
    on_boot   = true
  ...
  # Add DNS record to AdGuard after VM is created
    resource "null_resource" "add_dns_record" {
      depends_on = [proxmox_virtual_environment_vm.vikunja]
  ...
  home.file."${proj}/outputs.tf".text = ''
    output "name" {
      value = proxmox_virtual_environment_vm.vikunja.name
    }

    output "ipv4" {
      # first address of first NIC reported by the guest agent
      value = try(proxmox_virtual_environment_vm.vikunja.ipv4_addresses[0][0], null)
    }

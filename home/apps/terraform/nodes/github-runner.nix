{ pkgs, osConfig, ... }:
let
  proj = "infra/tf/github-runner-vm";

  defaults = {
    name    = "github-runner";
    ip_cidr = "10.0.0.87/24";
...
in {
  ...
  home.file."${proj}/variables.tf".text = ''
    ...
    variable "name"         { default = "github-runner" }
    variable "template_id"  { default = 9401 }
    variable "cpu_cores"    { default = 8 }
    variable "memory_mb"    { default = 16384 }
    variable "datastore"    { default = "local-lvm" }
    variable "disk_size_gb" { default = 112 }
    variable "ip_cidr"      { default = "10.0.0.87/24" }
  ...
  home.file."${proj}/main.tf".text = ''
      ...
      provisioner "local-exec" {
        command = <<-EOF
          ...
          op item create --category ssh \
            ...
            --url "ssh://${defaults.name}.host.internal" > /tmp/op_ssh_output.txt
    ...
    resource "proxmox_virtual_environment_vm" "github-runner" {
    name      = var.name
    node_name = var.node
    on_boot   = true
  ...
  # Add DNS record to AdGuard after VM is created
    resource "null_resource" "add_dns_record" {
      depends_on = [proxmox_virtual_environment_vm.github-runner]
  ...
  home.file."${proj}/outputs.tf".text = ''
    output "name" {
      value = proxmox_virtual_environment_vm.github-runner.name
    }

    output "ipv4" {
      # first address of first NIC reported by the guest agent
      value = try(proxmox_virtual_environment_vm.github-runner.ipv4_addresses[0][0], null)
    }

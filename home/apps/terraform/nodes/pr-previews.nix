{ pkgs, osConfig, ... }:
let
  proj = "infra/tf/pr-previews-vm";

  defaults = {
    name    = "pr-previews";
    ip_cidr = "10.0.0.86/24";
...
in {
  ...
  home.file."${proj}/main.tf".text = ''
    ...
    resource "proxmox_virtual_environment_vm" "pr-previews" {
    name      = var.name
    node_name = var.node
    on_boot   = true
  ...
  # Add DNS record to AdGuard after VM is created
    resource "null_resource" "add_dns_record" {
      depends_on = [proxmox_virtual_environment_vm.pr-previews]
  ...
  home.file."${proj}/outputs.tf".text = ''
    output "name" {
      value = proxmox_virtual_environment_vm.pr-previews.name
    }

    output "ipv4" {
      # first address of first NIC reported by the guest agent
      value = try(proxmox_virtual_environment_vm.pr-previews.ipv4_addresses[0][0], null)
    }

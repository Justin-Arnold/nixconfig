{ pkgs, osConfig, ... }:
let
  proj = "infra/tf/onepassword-connect-vm";

  defaults = {
    name        = "onepassword-connect";
    adguard_url = "http://10.0.0.1:3000";
    ip_cidr     = "10.0.0.70/24";
...
in {
  ...
  home.file."${proj}/variables.tf".text = ''
    ...
    variable "name"         { default = "onepassword-connect" }
    ...
    variable "disk_size_gb" { default = 60 }
    variable "ip_cidr"      { default = "10.0.0.70/24" }
  ...
  home.file."${proj}/main.tf".text = ''
    ...
    resource "proxmox_virtual_environment_vm" "onepassword-connect" {
      name      = var.name
      node_name = var.node
      on_boot   = true
    ...
    resource "null_resource" "add_dns_record" {
      depends_on = [proxmox_virtual_environment_vm.onepassword-connect]
  ...
  home.file."${proj}/outputs.tf".text = ''
    output "name" {
      value = proxmox_virtual_environment_vm.onepassword-connect.name
    }

    output "ipv4" {
      # first address of first NIC reported by the guest agent
      value = try(proxmox_virtual_environment_vm.onepassword-connect.ipv4_addresses[0][0], null)
    }
    ...
    output "ssh_public_key" {
      ...
      description = "Public SSH key from onepassword"
    }
  '';
